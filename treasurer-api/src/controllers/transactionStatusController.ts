...
export const bulkChangeStatus: RequestHandler = async (
  req,
  res,
  next,
): Promise<void> => {
  try {
    ...
    if (result.failed.length > 0) {
      res.status(207).json({
        success: true,
        data: result,
        message: `Bulk operation completed with ${String(result.successful.length)} successes and ${String(result.failed.length)} failures`,
      });
      return;
    }
...
export const completeReconciliation: RequestHandler = async (
  req,
  res,
  next,
): Promise<void> => {
