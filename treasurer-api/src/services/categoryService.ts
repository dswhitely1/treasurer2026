...
export async function createCategory(
  ...
  const depth = await calculateCategoryDepth(organizationId, input.parentId ?? null)

  if (depth > MAX_CATEGORY_DEPTH) {
    throw new AppError(`Category depth cannot exceed ${String(MAX_CATEGORY_DEPTH)} levels`, 400)
  }
...
export async function updateCategory(
  ...
  if (input.parentId !== undefined) {
    ...
    if (input.parentId) {
      const isCircular = await detectCircularReference(categoryId, input.parentId, organizationId)
      if (isCircular) {
    ...
    // Calculate new depth
    const newDepth = await calculateCategoryDepth(organizationId, input.parentId)
    ...
    if (maxDescendantDepth && maxDescendantDepth.depth + depthDelta > MAX_CATEGORY_DEPTH) {
      throw new AppError(
...
export async function moveCategory(
  ...
  return updateCategory(organizationId, categoryId, { parentId: input.newParentId })
}
  ...
  if (category._count.children > 0) {
    if (options.moveChildrenTo) {
      ...
      if (descendants.includes(options.moveChildrenTo)) {
        throw new AppError('Cannot move children to a descendant category', 400)
      }
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (options.moveChildrenTo) {
          for (const child of movedChildren) {
            const newDepth = await calculateCategoryDepth(organizationId, options.moveChildrenTo)
