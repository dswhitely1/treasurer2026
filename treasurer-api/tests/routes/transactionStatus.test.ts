...
describe('Transaction Status Routes', () => {
  ...
  describe('PATCH /api/organizations/:orgId/accounts/:accountId/transactions/:transactionId/status', () => {
    ...
    it('should return 404 for non-existent transaction', async () => {
      ...
      const response = await request(app)
        .patch(`/api/organizations/${orgId}/accounts/${accountId}/transactions/${fakeId}/status`)
        .set('Authorization', `Bearer ${token}`)
        .send({ status: 'CLEARED' })
  ...
  describe('GET /api/organizations/:orgId/accounts/:accountId/transactions/:transactionId/status/history', () => {
    ...
    it('should return 404 for non-existent transaction', async () => {
      const fakeId = '00000000-0000-0000-0000-000000000000'
      const response = await request(app)
        .get(`/api/organizations/${orgId}/accounts/${accountId}/transactions/${fakeId}/status/history`)
        .set('Authorization', `Bearer ${token}`)
  ...
  describe('GET /api/organizations/:orgId/accounts/:accountId/transactions/status/summary', () => {
    beforeEach(async () => {
      ...
      await request(app)
        .patch(`/api/organizations/${orgId}/accounts/${accountId}/transactions/${tx2Response.body.data.transaction.id}/status`)
        .set('Authorization', `Bearer ${token}`)
        .send({ status: 'CLEARED' })
    })
    ...
    it('should return 404 for non-existent account', async () => {
      const fakeId = '00000000-0000-0000-0000-000000000000'
      const response = await request(app)
        .get(`/api/organizations/${orgId}/accounts/${fakeId}/transactions/status/summary`)
        .set('Authorization', `Bearer ${token}`)
    ...
    it('should include transaction counts', async () => {
      ...
      expect(summary.overall.count).toBe(
        summary.uncleared.count + summary.cleared.count + summary.reconciled.count
      )
    })
