...
describe("Transaction Status Service", () => {
  ...
  describe("changeTransactionStatus", () => {
    ...
    it("should change status from CLEARED to RECONCILED", async () => {
      ...
      await changeTransactionStatus(orgId, accountId, transactionId, userId, {
        status: "CLEARED",
      });
    ...
    it("should throw error on invalid transition", async () => {
      await expect(
        ...
        "Invalid status transition from UNCLEARED to RECONCILED",
      );
    });
    ...
    it("should throw error if transaction not found", async () => {
      await expect(
        changeTransactionStatus(orgId, accountId, "non-existent-id", userId, {
          status: "CLEARED",
        }),
    ...
    it("should validate organization membership", async () => {
      ...
      await expect(
        changeTransactionStatus(wrongOrgId, accountId, transactionId, userId, {
          status: "CLEARED",
        }),
      ).rejects.toThrow("Account not found");
    });
  ...
  describe("bulkChangeTransactionStatus", () => {
    ...
    it("should continue processing after individual failures", async () => {
      ...
      expect(result.successful).toHaveLength(2);
      expect(
        result.successful.find((s) => s.transactionId === transactionId),
      ).toBeTruthy();
      expect(
        result.successful.find((s) => s.transactionId === transaction3Id),
      ).toBeTruthy();
    });

    it("should create audit trail for each successful change", async () => {
      await bulkChangeTransactionStatus(orgId, accountId, userId, {
        transactionIds: [transactionId, transaction2Id, transaction3Id],
        status: "CLEARED",
        notes: "Bulk clearing",
      });
    ...
    it("should throw error if account not found", async () => {
      await expect(
        bulkChangeTransactionStatus(orgId, "non-existent-account", userId, {
  ...
  describe("getTransactionStatusHistory", () => {
    ...
    it("should throw error if transaction not found", async () => {
      await expect(
        getTransactionStatusHistory(orgId, accountId, "non-existent-id"),
      ).rejects.toThrow("Transaction not found");
    });
    ...
    it("should validate organization membership", async () => {
      const wrongOrgId = "00000000-0000-0000-0000-000000000000";
      await expect(
        getTransactionStatusHistory(wrongOrgId, accountId, transactionId),
      ).rejects.toThrow("Account not found");
    });
  });
  ...
  describe("getReconciliationSummary", () => {
    ...
    it("should calculate overall totals correctly", async () => {
      ...
      expect(overallTotal).toBe(
        unclearedTotal + clearedTotal + reconciledTotal,
      );
